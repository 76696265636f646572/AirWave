<template>
  <aside class="min-h-0 h-full overflow-hidden rounded-xl border border-neutral-700 p-3 flex flex-col surface-panel">
    ...
    <ul class="min-h-0 flex-1 space-y-2 overflow-auto pr-1">
      <li v-for="speaker in speakers" :key="speaker.uid" class="rounded-md border p-2 playlist-card">
        <div class="flex items-center justify-between gap-2">
          <div>
            <div class="text-lg font-semibold leading-5">{{ speaker.name }}</div>
            <div class="text-xs text-muted">{{ speaker.ip }}</div>
          </div>
        ...
        <div class="mt-2 flex flex-wrap items-center gap-2">
          <select v-model="groupTargets[speaker.ip]" class="min-w-[130px] flex-1 rounded-md border px-2 py-1 text-sm surface-input">
